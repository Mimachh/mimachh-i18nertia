...
export function useTranslation() {
  ...
  const trans = (key: string, params: Record<string, string> = {}, choice?: number): string => {
    ...
    // Gérer le pluriel/singulier si choice est fourni
    if (choice !== undefined && translation.includes('|')) {
      const parts = translation.split('|');
      if (parts.length === 2) {
        translation = choice === 1 ? parts[0].trim() : parts[1].trim();
      }
    }
